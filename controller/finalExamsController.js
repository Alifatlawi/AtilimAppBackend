...
async function fetchEngFinalExams() {

    try {
        const url = "https://dersprogramiyukle.atilim.edu.tr/20232024finalbahar/m%C3%BCh/index_files/sheet001.htm"
        let reponse = await axios.get(url);
        const html = reponse.data;
        const $ = cheerio.load(html);
        const courses = [];
        ...
        const filePath = './public/FinalExams/EngFinalExams.json';
        saveDataToFile(courses, filePath);
    } catch (error) {
        console.log(error);
        throw error;
